...
export const SessionProvider = ({
  ...
  const mergeCartMetadata = (incomingItems: CartItem[], localItems: CartItem[]) => {
  ...
  useEffect(() => {
    ...
    const bootstrapCart = async () => {
      try {
        ...
        const mergedIds = [...new Set([...serverItems.map((item) => item.id), ...localIds])];
        ...
        const syncResponse = await cartClientService.sync(
          mergedIds.map((courseId) => {
            const localItem = mergedLocalItems.find((item) => item.id === courseId);
            return {
              courseId,
  ...
  useEffect(() => {
    ...
    const syncCart = async () => {
      try {
        ...
        const syncedItems = mergeCartMetadata(response.data?.items || [], cartItems);
        lastCartSignature.current = getSignature(syncedItems);
        replaceCartItems(syncedItems);
      } catch (error) {
