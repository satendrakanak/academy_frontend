...
export function ExamsView({ courses, examHistory }: ExamsViewProps) {
  ...
  return (
    <div className="space-y-8">
      <div className="academy-hero-gradient rounded-[30px] border border-white/10 p-6 text-white shadow-[0_34px_90px_-48px_rgba(15,23,42,0.55)] md:p-8">
      ...
      <div className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-[0_24px_60px_-42px_rgba(15,23,42,0.28)] dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(10,17,31,0.95),rgba(15,24,43,0.98))] dark:shadow-[0_32px_90px_-46px_rgba(0,0,0,0.68)] md:p-6">
        ...
        {upcomingExams.length === 0 ? (
          <div className="rounded-[26px] border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-sm text-slate-500 dark:border-white/10 dark:bg-white/6 dark:text-slate-300">
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            {upcomingExams.map(({ course, hasAttempted, progress }) => {
              ...
              return (
                <div
                  key={course.id}
                  className="rounded-[26px] border border-slate-200 bg-[linear-gradient(180deg,#ffffff,#f8fafc)] p-5 dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(10,17,31,0.92),rgba(16,26,46,0.98))]"
                >
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-300">
                    <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 dark:bg-white/8">
                      <Clock3 className="size-3.5" />
                      {course.exam?.timeLimitMinutes
                        ? `${course.exam.timeLimitMinutes} mins`
                        : "No time limit"}
                    </span>
                    <span className="rounded-full bg-white px-3 py-2 dark:bg-white/8">
