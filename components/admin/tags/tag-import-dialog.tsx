  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="!grid !w-[min(980px,calc(100vw-2rem))] !max-w-[min(980px,calc(100vw-2rem))] max-h-[min(720px,calc(100vh-3rem))] overflow-hidden rounded-[28px] border-[var(--brand-100)] bg-white p-0 shadow-[0_32px_100px_-50px_rgba(15,23,42,0.5)]"
      >
        ...
        <div className="space-y-6 overflow-hidden px-8 py-6">
          ...
          <ScrollArea className="h-[340px] rounded-2xl border border-slate-100 bg-white">
            <div className="min-w-[680px]">
              <div className="grid grid-cols-[1fr_1.7fr] gap-4 border-b border-slate-100 px-5 py-4 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                <span>Name</span>
              ...
              <div className="divide-y divide-slate-100">
                {previewRows.length ? (
                  previewRows.map((row, index) => (
                    <div
                      key={`${row.name}-${index}`}
                      className="grid grid-cols-[1fr_1.7fr] gap-4 px-5 py-4 text-sm text-slate-700"
                    >
                      <span>{row.name}</span>
